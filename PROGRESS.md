...
_이 파일은 `backlog.json`으로부터 자동 생성됩니다. 직접 편집하지 마세요. (생성: 2026-09-17T04:14:14.051Z)_

프로젝트: **ClaudeBridge**

## 요약

전체 47개 작업 — 완료 27, needs_info 3, 미결/보류 9, 할 일 8
...
| T-041 | 미결/보류 | 개인 액세스 토큰 발급 및 claudebridge login 커맨드 구현 (머신당 1회 로그인) |
...
| T-014 | 미결/보류 | 로컬 에이전트 설정 파일 설계 (project_id/tmux 세션명/Supabase 키) |
| T-015 | 완료 | AskUserQuestion 발생 감지 훅 작성 |
| T-016 | 완료 | Permission 프롬프트 발생 감지 훅 작성 |
| T-017 | 미결/보류 | 감지된 이벤트를 Supabase blocked_events에 기록하는 함수 작성 |
| T-018 | 미결/보류 | Supabase responses 테이블 폴링 로직 작성 (수초~10초 간격) |
| T-019 | 완료 | tmux 세션 존재 확인 및 생성 로직 작성 |
| T-020 | 미결/보류 | tmux send-keys로 AskUserQuestion 응답 주입하는 함수 작성 |
| T-021 | 미결/보류 | tmux send-keys로 Permission 승인/거부 응답 주입하는 함수 작성 |
...
| ID | 상태 | 제목 |
|---|---|---|
| T-022 | 미결/보류 | 선택지 텍스트→키 입력 매핑 규칙 정의 및 임의 텍스트 주입 차단 검증 |
...
| T-031 | 미결/보류 | 파싱한 토큰 사용량을 Supabase usage_logs에 적재하는 로직 작성 |
...
| T-033 | 미결/보류 | 막힌 이벤트 발생 시 웹 푸시 발송 로직 작성 |
